...
function calc() {
  var persons = document.querySelectorAll('.counter-block-input')[0],
      ...
      // получаем значение у опции места
  total = 0;
...
function forms() {
  ...
  var phone = document.querySelectorAll('input[name="phone"]'); // получаем поля с телефонами
  ...
  secondForm.addEventListener('submit', function (event) {
    ...
    postData(secondForm).then(function () {
      return statusMassege.innerHTML = message.success;
    }).catch(function () {
      return statusMassege.innerHTML = message.failure;
    });
...
function modals() {
  var descBtn = document.querySelectorAll('.description-btn'),
      more = document.querySelector('.more'),
      overlay = document.querySelector('.overlay'),
      popupClose = document.querySelector('.popup-close');
  descBtn.forEach(function (element) {
